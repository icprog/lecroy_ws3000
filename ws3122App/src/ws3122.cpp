...
static const char *driverName="WS3122";
void simTask(void *drvPvt);


/** Constructor for the WS3122 class.
  * Calls constructor for the asynPortDriver base class.
  * \param[in] portName The name of the asyn port driver to be created.
  * \param[in] maxPoints The maximum  number of points in the volt and time arrays */
WS3122::WS3122(const char *portName, int maxPoints) 
                    ...
                    0) /* Default stack size*/    
{
    ...
    status = (asynStatus)(epicsThreadCreate("WS3122Task",
...
asynStatus WS3122::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
...
/** Called when asyn clients call pasynFloat64->write().
  ...
  * \param[in] pasynUser pasynUser structure that encodes the reason and address.
  * \param[in] value Value to write. */
asynStatus WS3122::writeFloat64(asynUser *pasynUser, epicsFloat64 value)
{
    int function = pasynUser->reason;
    asynStatus status = asynSuccess;
    int run;
...
asynStatus WS3122::readFloat64Array(asynUser *pasynUser, epicsFloat64 *value, 
...
asynStatus WS3122::readEnum(asynUser *pasynUser, char *strings[], int values[], int severities[], size_t nElements, size_t *nIn)
{
    int function = pasynUser->reason;
    size_t i;
...
/** EPICS iocsh callable function to call constructor for the WS3122 class.
  * \param[in] portName The name of the asyn port driver to be created.
  * \param[in] maxPoints The maximum  number of points in the volt and time arrays */
int WS3122Configure(const char *portName, int maxPoints)
{
    new WS3122(portName, maxPoints);
    return(asynSuccess);
}
...
static const iocshArg * const initArgs[] = {&initArg0,
                                            &initArg1};
static const iocshFuncDef initFuncDef = {"WS3122Configure",2,initArgs};
static void initCallFunc(const iocshArgBuf *args)
{
    WS3122Configure(args[0].sval, args[1].ival);
}

void WS3122Registrar(void) {
    iocshRegister(&initFuncDef,initCallFunc);
}

epicsExportRegistrar(WS3122Registrar);
